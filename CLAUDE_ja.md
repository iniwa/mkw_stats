...
- Claude Code は原則 **auto モード**（自動モデル選択）で動かす。固定の統括役/サブエージェント分担は設けない。
- 設計判断は Codex が担当し、handoff は追加の設計判断なしに実装が完結する粒度で書かれる前提。Claude Code は実装・検証・報告を直接行う。
- サブエージェントは既定ではなく任意の手段（例: 広範囲の並行調査）。使う場合は、目的、対象ファイル、制約、非目標、期待する報告内容を狭く明示する。
- モデルによらず、設計意図の変更、作業範囲の拡大、依存関係の追加、ビルド/デプロイ/外部公開方式の変更、秘密情報やローカル設定への接触は単独で行わない。アーキテクチャ判断は Codex に戻す。
- 想定したモデルやツールが使えない場合は、利用可能なもので継続し、その制約を完了報告に含める。
