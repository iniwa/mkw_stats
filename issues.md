...
### A-7. プレイ中のアノテーション追加・マップアイコン設置 (P2・新機能) — 元: ユーザー追加 2026-06-16 ✅
- [x] コース確認に「🗺 アノテーション/アイコンを編集」トグルを追加し、既存 `AnnotationEditor` を折りたたみ表示
- クリック配置・pin/icon/arrow/text/area・作成/編集/削除がプレイ中フローでそのまま利用可能
- 親から `routes`/`courseMap` を渡し、`annotations` state を共有(TargetImage の表示にも反映)
- 対象: `PlayingView.tsx` `SelectionConfirm`
